...
import {
  ...
  SteamApiClient,
  SteamGame,
} from '@perion.steam.challenge/steam-api';
...
export type GetSteamGamesApiResponse =
  | GetSteamGamesApiSuccess
  | GetSteamGamesApiError;

export async function getSteamGames(
  formData: FormData
): Promise<GetSteamGamesApiResponse> {
  try {
    const { idValue, idType } = applySchema(
      getSteamGamesInputSchema,
    ...
    const steamGamesSortedByPlaytime = sortBy(
      steamGames.games,
      (game) => game.playtime_forever ?? 0
    );
    const totalNumberOfGamesOwned = steamGames.game_count;
    const mostPlayedGame =
      steamGamesSortedByPlaytime[steamGamesSortedByPlaytime.length - 1];
    const playtimeAcrossAllGames = games.reduce(
